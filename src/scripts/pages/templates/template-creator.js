  ...
  <div class="story-item" role="article" aria-labelledby="title-${story.id}">
    <div class="story-item__header">
      <img class="story-item__image" 
           ...
           onerror="this.onerror=null; this.src='/images/placeholder.png'; this.classList.add('fallback-image');"
           loading="lazy">
    </div>
...
const createStoryDetailTemplate = (story) => `
  <div class="story-detail" role="article" aria-labelledby="detail-title">
    ...
    ${story.lat && story.lon ? `
      <div class="map-wrapper online-only">
        <div id="detail-map" class="story-detail__map" tabindex="0" role="application" aria-label="Peta lokasi cerita"></div>
      </div>
      <div class="offline-only map-fallback">
        <p>Peta lokasi tidak tersedia saat offline. Koordinat: ${story.lat}, ${story.lon}</p>
      </div>
    ` : ''}
  </div>
`;
